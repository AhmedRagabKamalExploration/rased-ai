...
export class IdentityManager {
    ...
    public async initialize(): Promise<void> {
        try {
            ...
            console.log("[SDK] IdentityManager initialized. Device ID:", this._deviceId);
        } catch (error) {
            console.error("[SDK] IdentityManager failed to initialize. Crypto API may be unavailable.", error);
    ...
    private getDB(): Promise<IDBDatabase> {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = () => request.result.createObjectStore(KEY_NAME);
